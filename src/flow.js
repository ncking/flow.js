...
export function Flow(opts) {
   ...
   */
  this.onDrop = function (event, fileMeta) {
    ...
    if (dataTransfer.items && dataTransfer.items[0] &&
      ...
      $.webkitReadDataTransfer(event, fileMeta);
    } else {
      $.addFiles(dataTransfer.files, event, fileMeta);
    }
  };
...
Flow.prototype = {
   ...
   * @param event
   */
  webkitReadDataTransfer: function (event, fileMeta) {
    var $ = this;
   ...
   */
  addFile: function (file, event, fileMeta) {
    this.addFiles([file], event,  fileMeta);
  },
   ...
   * @param {Event} [event] event is optional
   */
  addFiles: function (fileList, event, fileMeta) {
    var files = [];
    each(fileList, function (file) {
      ...
      if ((!ie10plus || ie10plus && file.size > 0) && !(file.size % 4096 === 0 && (file.name === '.' || file.fileName === '.'))) {
        ...
        if (this.opts.allowDuplicateUploads || !this.getFromUniqueIdentifier(uniqueIdentifier)) {
          var f = new FlowFile(this, file, uniqueIdentifier, fileMeta);
 ...
 * @constructor
 */
function FlowFile(flowObj, file, uniqueIdentifier, fileMeta = {}) {
...
FlowChunk.prototype = {
  ...
  prepareXhrRequest: function (method, isTest, paramsMethod, blob) {
    ...
    query = extend(query || {}, this.fileObj.fileMeta, this.getParams());
